  ...
  Future<void> _addPatient() async {
    ...
    final patient=await showDialog<Patient>(context:context,builder:(dialogContext)=>StatefulBuilder(builder:(context,setDialogState)=>AlertDialog(
      ...
      content:SizedBox(width:520,child:SingleChildScrollView(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        Text('Identificação',style:Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight:FontWeight.w800)),
        ...
        const SizedBox(height:10),
        DropdownButtonFormField<String>(initialValue:sex,decoration:const InputDecoration(labelText:'Sexo'),items:const ['Feminino','Masculino','Outro','Prefere não informar'].map((v)=>DropdownMenuItem(value:v,child:Text(v))).toList(),onChanged:(v)=>setDialogState(()=>sex=v)),
        ...
        Text('Dados físicos',style:Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight:FontWeight.w800)),
        Row(children:[Expanded(child:TextField(controller:weight,keyboardType:const TextInputType.numberWithOptions(decimal:true),inputFormatters:[FilteringTextInputFormatter.allow(RegExp(r'[0-9,.]'))],decoration:const InputDecoration(labelText:'Peso (kg)'))),const SizedBox(width:12),Expanded(child:TextField(controller:height,keyboardType:const TextInputType.numberWithOptions(decimal:true),inputFormatters:[FilteringTextInputFormatter.allow(RegExp(r'[0-9,.]'))],decoration:const InputDecoration(labelText:'Altura (cm)',hintText:'175')))]),
        const SizedBox(height:18),
        Text('Informações clínicas',style:Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight:FontWeight.w800)),
        TextField(controller:diagnosis,maxLines:2,decoration:const InputDecoration(labelText:'Diagnóstico médico informado',hintText:'Diagnóstico/hipótese do encaminhamento')),
        TextField(controller:complaint,maxLines:2,decoration:const InputDecoration(labelText:'Queixa principal')),
        TextField(controller:comorbidities,maxLines:2,decoration:const InputDecoration(labelText:'Comorbidades')),
        TextField(controller:medications,maxLines:2,decoration:const InputDecoration(labelText:'Medicamentos em uso')),
        TextField(controller:allergies,maxLines:2,decoration:const InputDecoration(labelText:'Alergias')),
        TextField(controller:surgeries,maxLines:2,decoration:const InputDecoration(labelText:'Cirurgias / histórico relevante')),
        TextField(controller:precautions,maxLines:2,decoration:const InputDecoration(labelText:'Restrições e precauções')),
        TextField(controller:referring,decoration:const InputDecoration(labelText:'Profissional que encaminhou')),
        TextField(controller:notes,maxLines:4,decoration:const InputDecoration(labelText:'Observações gerais')),
    ...
    if(patient==null)return;await widget.database.addPatient(patient);if(mounted)setState(_reload);
  }

  Future<void> _deletePatient(Patient p)async{
    if(p.id==null)return;
    final confirmed=await showDialog<bool>(context:context,builder:(context)=>AlertDialog(title:const Text('Excluir paciente permanentemente?'),content:Text('Excluir ${p.name} também apagará todas as avaliações, evoluções, medições e testes vinculados. Esta ação não pode ser desfeita.'),actions:[TextButton(onPressed:()=>Navigator.pop(context,false),child:const Text('Cancelar')),FilledButton(style:FilledButton.styleFrom(backgroundColor:Theme.of(context).colorScheme.error),onPressed:()=>Navigator.pop(context,true),child:const Text('Excluir permanentemente'))]));
    if(confirmed==true){await widget.database.deletePatient(p.id!);if(mounted){setState(_reload);ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('${p.name} foi excluído.')));}}
  }
